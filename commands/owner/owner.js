...
module.exports = {
    name: 'owner',
    description: "Permet de owner un utilisateur.",
    options: [
        {
            name: 'type',
    ...
    run: async (client, interaction) => {
        ...
        if (type === 'add') {
            if (!config.owners.includes(interaction.user.id) && interaction.user.id !== interaction.guild.ownerId) return interaction.reply({ embeds: [new EmbedBuilder().setDescription(`> \u{274C} Vous devez etre \`buyer\` pour executer cette commande.`).setColor("#ED4245")], ephemeral: true })
            ...
            if (db.get(`Owner_${interaction.guild.id}-${user.id}`) === null) {
                ...
                interaction.reply({
                    embeds: [new EmbedBuilder()
                        .setDescription(`> \u{2705} ${user} a ete ajoute dans la liste des **owners**.`)
                        .setColor("#57F287")
                ...
                interaction.reply({ embeds: [new EmbedBuilder().setDescription(`> \u{274C} Cet utilisateur est deja owner.`).setColor("#ED4245")], ephemeral: true })
            }
        }
        if (type === 'remove') {
            if (!config.owners.includes(interaction.user.id) && interaction.user.id !== interaction.guild.ownerId) return interaction.reply({ embeds: [new EmbedBuilder().setDescription(`> \u{274C} Vous devez etre \`buyer\` pour executer cette commande.`).setColor("#ED4245")], ephemeral: true })
            ...
            if (db.get(`Owner_${interaction.guild.id}-${user.id}`) !== null) {
                ...
                interaction.reply({
                    embeds: [new EmbedBuilder()
                        .setDescription(`> \u{2705} ${user} a ete retire de la liste des **owners**.`)
                        .setColor("#57F287")
                        .setFooter({ text: `Par ${interaction.user.tag}`, iconURL: interaction.user.displayAvatarURL() })
                        .setTimestamp()
                    ]
                ...
                interaction.reply({ embeds: [new EmbedBuilder().setDescription(`> \u{274C} Cet utilisateur n'est pas owner.`).setColor("#ED4245")], ephemeral: true })
            }
        }
        if (type === 'list') {
            if (!config.owners.includes(interaction.user.id) && interaction.user.id !== interaction.guild.ownerId) return interaction.reply({ embeds: [new EmbedBuilder().setDescription(`> \u{274C} Vous devez etre \`buyer\` pour executer cette commande.`).setColor("#ED4245")], ephemeral: true })
            if (user !== null) {
                if (db.get(`Owner_${interaction.guild.id}-${user.id}`) !== null) {
                    ...
                    interaction.reply({
                        ephemeral: true,
                        embeds: [new EmbedBuilder()
                            .setAuthor({ name: 'Informations Owner', iconURL: user.displayAvatarURL() })
                            .setDescription(`> \u{1F451} Details de l'owner **${user.tag}**`)
                            .addFields(
            ...
            if (user === null) {
                ...
                for (let i in owner) {
                    ...
                    content += `\n> \u{1F451} \`${userData.tag}\` \u{2500} \`${userData.id}\``
                }
                interaction.reply({
                    embeds: [new EmbedBuilder()
                        .setAuthor({ name: 'Liste des Owners', iconURL: interaction.guild.iconURL() })
                        .setDescription(content || '> Aucun owner configure.')
                        .setColor("#5865F2")
        ...
        if (type === 'clear') {
            ...
            const embed = new EmbedBuilder()
                .setDescription(`> \u{26A0}\u{FE0F} Voulez-vous supprimer **tous les owners** du serveur ?`)
                .setColor("#FEE75C");
            ...
            collector.on('collect', async (i) => {
                if (i.customId === 'yes') {
                    const ownerKeys = db.all().filter((data) => data.ID.startsWith(`Owner_${interaction.guild.id}-`)).map((data) => data.ID);
                    ...
                    const embed = new EmbedBuilder()
                        .setDescription(`> \u{2705} **${zzz}** owner(s) ont ete supprimes avec succes.`)
                        .setColor("#57F287");
