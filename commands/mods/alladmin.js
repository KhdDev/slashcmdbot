...
module.exports = {
    name: "alladmin",
    description: "Permet de voir la liste de tout les administrateur du serveur.",
    ...
    run: async (client, interaction) => {
        ...
        const str_filtrer = interaction.guild.members.cache.filter(member => member.permissions.has(PermissionFlagsBits.Administrator) && !member.user.bot)
        if (str_filtrer.size === 0) {
            return interaction.reply({ embeds: [new EmbedBuilder().setDescription(`> \u{2705} Aucun administrateur trouve sur ce serveur.`).setColor("#57F287")], ephemeral: true });
        }

        let p1 = 15;
        let page = 0;
        const adminList = str_filtrer.map(user => `> \u{1F6E1}\u{FE0F} <@${user.user.id}> \u{2500} \`${user.user.id}\``)
        ...
        const banEmbed = new EmbedBuilder()
            .setAuthor({ name: 'Liste des Administrateurs', iconURL: interaction.guild.iconURL() })
            .setDescription(pages[page])
            .setColor("#5865F2")
            .setFooter({ text: `Total : ${str_filtrer.size} admin(s) | Page ${page + 1}/${pages.length}` })
            .setTimestamp()
        ...
        collector.on('collect', async interaction => {
            ...
            if (interaction.customId === 'suivant') {
                ...
                banEmbed.setFooter({ text: `Total : ${str_filtrer.size} admin(s) | Page ${page + 1}/${pages.length}` });
